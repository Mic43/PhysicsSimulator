...
type StackScene() =
    inherit SceneBase(StackScene.createSimulator ())
    ...
    static member private createVerticalStack position height boxSize massValue =
        [ 0 .. height - 1 ]
        |> List.map (fun i ->
    ...
    static member private createSimulator () =
        let rigidBodyPrototypes =
            ...
            @ StackScene.createVerticalStack ((0.0, 0.0, 1.5) |||> Vector3D.create) 3 1.0 mass
